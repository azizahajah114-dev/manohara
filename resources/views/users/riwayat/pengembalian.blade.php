...
<body class="bg-[#D1D8BE] font-sans">
    ...
    <div class="max-w-4xl mx-auto py-10 px-5">
        <h2 class="max-4xl font-bold text-[#555] mb-8">Upload Bukti Pengembalian</h2>
...
@endif
            @if (!$sewa->pengembalian || !$sewa->pengembalian->bukti_pengembalian)
                <form action="{{route('riwayat.uploadBuktiPengembalian', $sewa->id)}}" method="POST" enctype="multipart/form-data">
                  @csrf
                  <label for="bukti_pembayaran" class="block text-sm font-medium text-gray-700 mb-1">Upload Bukti Pengembalian:</label>
                  <input type="file" name='bukti_pengembalian' id="bukti_pembayaran" accept="image/*" required class="block w-full text-sm text-gray-500
                    file:mr-4 file:py-2 file:px-4
                    file:rounded-full file:border-0
                    file:text-sm file:font-semibold
                    file:bg-[#819A91] file:text-white
                    hover:file:bg-[#6b817a]
                    "/>
